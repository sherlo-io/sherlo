...
[
Bardzo krótki tekst wstepu ktory wspomina o tym ze:

- ten przyklad/workflow pracuje z "tests on app builds with bundled JavaScript code"
- jest to example RN + SB (np. cos w stylu "Minimal React Native + Storybook setup")
- zawiera github akcje

Co istotne to pierwszy bullet ktory podalem powyzej ZAWSZE bedzie sie roznil miedzy example'ami, a dwa ostatnie zawsze beda takie same - takze powinnismy bardziej jakos wyroznic tekst o tym czym jest ten example/workflow
]

[
Mysle ze lepiej tez bylo by TUTAJ wstawiac nasz graf reprezentujacy jak wyglada dany workflow, zeby user ODRAZU rozumial jak ten workflow dziala
(mam na mysli ze tutaj powinien sie znalezc nasz aktualny Workflow Visualization)
]
...
[Wolalbym jakbysmy przedstawiali takie rzeczy zwiazane z kodem w takiej formie:
...
- i powinnismy miec komentarz nad KAZDA linia kodu + np powinnismy miec tez comment z "(or `npm install`)"
  ]
...
- Tez postarajmy sie (przynajmniej tyle ile mozemy) pisac instrukcje w formie kodu
- jednak niech github actions bedzie pierwsze (i chyba lepiej zeby bylo pisane "via GitHub Actions" (z malej litery)?)
  ]
...
- Skrocmy troche opisy
- niech kazda nazwa file bedzie jednoczenie linkiem do danego pliku
- zmienmy kolejnosc: sherlo.config, storybook component, app, github
- moze sprobujmy przeniesc tekst poswiecony sherlo init do "Additional Notes"?
  ]
