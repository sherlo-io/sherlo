...
describe('the masker folds every volatile class the tool prints', () => {
  it.todo('every volatile class the tool prints is folded to one placeholder, by class, on a posed screen and on a live screen alike');
  it.todo('folds a token wherever it appears - after --token, after token:, in a build url, in an Authorization header');
  it.todo('folds a build url, a size in megabytes, a duration, the time since a build and a base fingerprint');
  ...
  it.todo('strips the terminal artifacts a live run leaves - spinner redraws and the cursor-show sequence');
...
describe('sherlo mask - the same folding for a screen the tool did not print itself', () => {
  it.todo('reads a screen on stdin and prints it folded, byte for byte what applyMasks would have produced');
