...
describe.each(SB_VERSIONS)('Error: App.tsx render-time crash - %s', (sbVersion) => {
  if (!isVariantCompatible('js-app-tsx-crash', sbVersion)) {
    test.skip(`js-app-tsx-crash not compatible with ${sbVersion}`, () => {});
    return;
  }
  ...
  beforeAll(async () => {
    ...
    const build = buildApp({ appName: APP_NAME, sbVersion, variantName: 'js-app-tsx-crash', platform: PLATFORM });
  ...
  test('app crash: JS_ERROR captured before STORYBOOK_LOADED', async () => {
    // Arrange
    ...
    expectJsError(protocol, {
      messageContains: 'Intentional JS app-tsx crash via IntentionalCrasher',
    });
