...
describe.each(SB_VERSIONS)('Error: JS module-eval crash - %s', (sbVersion) => {
  if (!isVariantCompatible('js-module-eval-crash', sbVersion)) {
    test.skip(`js-module-eval-crash not compatible with ${sbVersion}`, () => {});
    return;
  }
  ...
  beforeAll(async () => {
    ...
    const build = buildApp({ appName: APP_NAME, sbVersion, variantName: 'js-module-eval-crash', platform: PLATFORM });
  ...
  test('app crash: JS_ERROR captured before app initialization', async () => {
    // Arrange
    ...
    expectJsError(protocol, {
      messageContains: 'Intentional JS crash before app initialization',
    });
