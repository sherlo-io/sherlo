...
describe.each(SB_VERSIONS)('Empty storybook - zero stories - %s', (sbVersion) => {
  if (!isVariantCompatible('empty-storybook', sbVersion)) {
    test.skip(`empty-storybook not compatible with ${sbVersion}`, () => {});
    return;
  }
  ...
  beforeAll(async () => {
    ...
    const build = buildApp({ appName: APP_NAME, sbVersion, variantName: 'empty-storybook', platform: PLATFORM });
  ...
  test('discovery: empty snapshots array when no stories found', async () => {
    // Arrange
    ...
    expectStartItem(start, { snapshots: [] });
    expectNoErrors(protocol);
    expectSdkLog(log, 'start testing session', { storiesCount: 0 });
  }, 5 * 60_000);
});
